...
userspace via a `signal` which we can see being done in `do_fpe_trap`.
The excption becomes a `SIGFPE` signal and exception address will be set
...
Then upon return from kernel space to user space the path is `do_fpe_trap` >
