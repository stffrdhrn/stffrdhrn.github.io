...
The patch to *allow FPCSR access in user mode* was trivial, but required some
code structure changes making to diff look bigger than it really was.

The next patch to *properly set to exception PC address* was required because up
until now user space FPU exceptions were not expected in QEMU as openrisc did
not have any software that supported user space FPU instructions.  The exception
PC address stores the program counter (PC) that was executing when an exception
occurs.  The OS can then handle and exception and report this address to
userspace and/or return to this address when exception handling complete.  In
...
The reason QEMU was not setting the corrent exception address is due to the way
