...
It takes a symbol (memory address) and makes it usable in our CPU by generating code
sequences that are possible on our CPU to load that address into a register
based on the symbol properties.

A snippet from this function is below.  The argument `x` represents our input
symbol (memory address) that we need to make usable by our CPU.  This code uses
GCC internal API's to emit RTX code sequences.
...
Once RTX is generated by legitmize address and [GCC passes]({% post_url 2018-06-03-gcc_passes %})
...
found in `opcodes/or1k-asm.c`.  The function `parse_reloc()` is the direct ounterpart of GCC's `print_reloc()`
...
The API definition is split accross multiple files which include:
...
function is called to fill in the relocation holes in the ouput binary `.text`
...
This can be read as:
 ...
 - Otherwise the current symbol is global:
   - We use the `RELOC_FOR_GLOBAL_SYMBOL()` macro to initalize `relocation`.
...
This can be read as:
   ...
   - Mark the symbol as using a TLS got entry, this `offset |= 3` trick is
     possible becuase on 32-bit machines we have 2 lower bits free.  This
...
This is done in the following files and functions.
 ...
 - `elf/rtld.c` - implements [init_tls()](https://github.com/stffrdhrn/or1k-glibc/blob/or1k-port-1/elf/rtld.c#L739) which initializes the tls datastructures.
...
TLS datastructure that looks as follows:
...
The symbols and macros defined in `sysdeps/or1k/nptl/tls.h` are:
 ...
 - `__thread_self` - a symbol represending the current thread always
 - `TLS_DTV_AT_TP` - used thoughout the tls code to adjust offsets
 - `TLS_TCB_AT_TP` - used thoughout the tls code to adjust offsets
